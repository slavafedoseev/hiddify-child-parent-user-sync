# Hiddify Manager - Child→Parent User Synchronization v3.0
...
### 🕐 Синхронизация last_online (Child ↔ Parent) — v3.0

- ✅ **Двунаправленная синхронизация** `last_online` — выигрывает более свежее значение
- ✅ **Если пользователь активен на этом child** → обновляет parent
- ✅ **Если пользователь активен на другом child** → обновляет локальную БД
- ✅ Parent всегда видит актуальное время последнего подключения со всех серверов

### 🛡 Стабилизация Celery background tasks — v3.0

- ✅ **Автопатч PendingRollbackError** — исправляет архитектурный баг Hiddify Manager
- ✅ **Идемпотентный патчер** — безопасно переприменяется при каждом старте сервиса
- ✅ **Переживает обновления Hiddify** — через systemd drop-in (не модифицирует исходники)
- ✅ Предотвращает остановку сбора статистики трафика из-за залипшей SQLAlchemy-сессии
...
├── src/
│   ├── stable_sync.py                 # Основной скрипт синхронизации (v4.1)
│   ├── activate_new_users_direct.py   # Мгновенная активация в Xray
│   ├── sync_health_api.py             # HTTP API мониторинга
│   └── hiddify-patch-celery-rollback.py  # Автопатч Celery PendingRollbackError
├── systemd/
│   ├── hiddify-child-sync.service     # Systemd сервис синхронизации
│   ├── hiddify-child-sync.timer       # Systemd таймер (каждые 5 мин)
│   ├── hiddify-sync-api.service       # Systemd сервис API
│   └── celery-rollback-patch.conf     # Drop-in для автоприменения патча Celery
└── docs/
    └── CHANGELOG.md                    # Устаревший changelog (см. корневой)
```
